...
class JSPDXTMMEventsWidget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		for ($i = 1; $i <= $amount; $i++) {
			...
			$item_dates[$i] = $instance['item_date' . $i];
				...
				<?php foreach ($items as $num => $item) : 
					if (!empty($item)) :
						if (empty($item_links[$num])) :
							if($item_dates[$num]) :
								echo("<li class='jspdx-tmm-events-li'><span class='" . $item_classes[$num] . "'>" . $item . "</ br>" . $item_dates . "</span></li>");
							else :
							...
							if($item_targets[$num]) :
								if($item_dates[$num]) :
									echo("<li class='jspdx-tmm-events-li'><a href='" . $item_links[$num] . "' target='_blank' class='" . $item_classes[$num] . "'>" . $item . "</a></ br>" . $item_dates . "</li>");
								else:
									echo("<li class='jspdx-tmm-events-li'><a href='" . $item_links[$num] . "' target='_blank' class='" . $item_classes[$num] . "'>" . $item . "</a></li>");
								endif;
							else :
								if($item_dates[$num]) :
									echo("<li class='jspdx-tmm-events-li'><a href='" . $item_links[$num] . "'' class='" . $item_classes[$num] . "'>" . $item . "</a></ br>" . $item_dates . "</li>");
								else:
									if($item_dates[$num]) :
	...
	function update( $new_instance, $old_instance ) {
		...
		for ($i = 1; $i <= $instance['amount']; $i++) {
			...
			$instance['item_dates' . $i] = $new_instance['item_date' . ($j+$i)];
...
?>
		...
		<?php foreach ($items as $num => $item) : ?>
			...
			<li>
				...
				<label for="<?php echo $this->get_field_id('item_date' . $num); ?>">Event date(s):</label>
				<input class="widefat" id="<?php echo $this->get_field_id('item_date' . $num); ?>" name="<?php echo $this->get_field_name('item_date' . $num); ?>" type="text" value="<?php echo esc_attr($item_dates[$num]); ?>" />
				<label for="<?php echo $this->get_field_id('item_class' . $num); ?>">Custom Style Class:</label>

				<!-- RADIO BUTTONS FOR CLASS -->
				<label for="<?php echo $this->get_field_id('item_class' . $num); ?>"><input type="radio" id="<?php echo $this->get_field_id('item_class' . $num); ?>" name="<?php echo $this->get_field_name('item_class' . $num); ?>" value="" <?php checked($item_classes[$num], ''); ?> /> None</label></ br>
		...
		<?php //Additional form fields to add one more item ?>
			...
			<li>
				...
				<label for="<?php echo $this->get_field_id('item_date' . $new_amount); ?>">Event date(s):</label>
				<input class="widefat" id="<?php echo $this->get_field_id('item_date' . $new_amount); ?>" name="<?php echo $this->get_field_name('item_date' . $new_amount); ?>" type="text" value="" />
